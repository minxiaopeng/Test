...
// global variables used by the filesystem
lfs_t extern_lfs;
                                    ...
                                    lfs_off_t off, void *buffer, lfs_size_t size)
{
    ...
    spi_flash_read_data((block * c->block_size + off), (uint8_t *)buffer, size);
    return 0;
}
                                    ...
                                    lfs_off_t off, const void *buffer, lfs_size_t size)
{
    ...
    ASSERT(block < c->block_count);
    spi_flash_write_data((block * c->block_size + off), (uint8_t *)buffer, size);
    return 0;
}
...
{
    ASSERT(block < c->block_count);
    spi_flash_erase_4kSector(block * c->block_size);
    return 0;
}
...
// entry point
void spi_flash_littlefs_mount(void)
{
    // mount the filesystem
    //int i; 
    //uint32_t buf_len = 0;
    //uint8_t test_buf[1024] = {'\0'};
    /*[HM003-62] add factory test code by lizhiye at 20191023 begin*/
    //lfs_file_t file;
    /*[HM003-62] add factory test code by lizhiye at 20191023 end*/
    int err = lfs_mount(&extern_lfs, &cfg);
    if (err)        //reformat if we can't mount the filesystem this should only happen on the first boot
    {
     NRF_LOG_INFO("spi_flash_littlefs_mount failed  ");
        spi_flash_erase_full_chip();
        lfs_format(&extern_lfs, &cfg);
        lfs_mount(&extern_lfs, &cfg);
    }
	 NRF_LOG_INFO("spi_flash_littlefs_mount success  ");
}

/*[HM003-155]  km-goldie,add eraser spi flash chip by tangmingyu at 20200119 start*/
int spi_flash_eraser_and_remount(void)
{
    int rev = -1;

    lfs_unmount(&extern_lfs);
    spi_flash_erase_full_chip();
    lfs_format(&extern_lfs, &cfg);
    rev = lfs_mount(&extern_lfs, &cfg);
    return (rev);
}
/*[HM003-155]  km-goldie,add eraser spi flash chip by tangmingyu at 20200119 end*/


/*[HM003-128] add report buffer function by ganxiao at 20191223 start*/
u32 lfs_free_spcae(lfs_t *lfs)
{
    lfs_size_t use_space = 0;
    use_space = lfs_fs_size(lfs);
    return cfg.block_count - use_space;
}

void printLfsFreeSpace(void)
{
    u32 free_spcae_size;
    free_spcae_size = lfs_free_spcae(&extern_lfs);
    NRF_LOG_INFO("lfs free space = %d", free_spcae_size);
}

void printLfsFileInfo(void *filepath)
{
    struct lfs_info lfs_file_info = {0};

    memset(&lfs_file_info, 0x00, sizeof(lfs_file_info));

    lfs_stat(&extern_lfs, filepath, &lfs_file_info);
    NRF_LOG_INFO("printFileInfo type = %d size = %d  filename = %s", lfs_file_info.type, lfs_file_info.size,
                 lfs_file_info.name);
}
/*[HM003-128] add report buffer function by ganxiao at 20191223 end*/
/*[HYN001-22] add littlefs filesystem for spi flash by yangguangfu at 20201207 begin*/
#if 0
/*[HM003-165] Add AT+SPIFLASHLIFETEST command by ganxiao at 20200306 begin*/
extern lfs_file_t g_lfs_file;

int spi_flash_life_test(void)
{
    int ret = -1;
    u8 testFile[] = {"test"};
    u8 testString[] = {"staging-edi.kiotics.com/edi-v1/jaxrs"};
    u32 file_size = 0;
    char buffer[40] = {0};

    while (g_spi_life_test_cycles > 0)
    {
        printf("SPI FLASH life test cycle %d start\r\n", g_spi_life_test_cycles);

        ret = lfs_file_open(&extern_lfs, &g_lfs_file, testFile, LFS_O_RDWR | LFS_O_CREAT | LFS_O_TRUNC);
        if (ret != LFS_ERR_OK)
            break;

        lfs_file_write(&extern_lfs, &g_lfs_file, testString, strlen(testString));
        lfs_file_close(&extern_lfs, &g_lfs_file);

        ret = lfs_file_open(&extern_lfs, &g_lfs_file, testFile, LFS_O_RDONLY);
        if (ret != LFS_ERR_OK)
            break;

        memset(buffer, 0, sizeof(buffer));
        file_size = lfs_file_size(&extern_lfs, &g_lfs_file);
        lfs_file_seek(&extern_lfs, &g_lfs_file, 0, LFS_SEEK_SET);
        lfs_file_read(&extern_lfs, &g_lfs_file, buffer, file_size);
        lfs_file_close(&extern_lfs, &g_lfs_file);

        ret = lfs_remove(&extern_lfs, testFile);
        if (ret != LFS_ERR_OK)
            break;
        ...
        printf("SPI FLASH life test cycle %d passed\r\n", g_spi_life_test_cycles);

        if (g_spi_life_test_cycles == 4294967295)
        {
            printf("SPI FLASH life test finish\r\n");
            return LFS_ERR_OK;
        }

        g_spi_life_test_cycles++;

        /*[HM003-165] fix AT+SPIFLASHLIFETEST command by ganxiao at 20200319 begin*/
        if (g_spi_life_test_cycles % 1000 == 0)
        {
            watchdog_feed_ML51();
            NRF_LOG_INFO("spi_flash_life_test feed watchdog at test cycle %d", g_spi_life_test_cycles);
        }
        /*[HM003-165] fix AT+SPIFLASHLIFETEST command by ganxiao at 20200319 end*/

        nrf_delay_ms(10);
    }

    printf("SPI FLASH life test cycle %d failed\r\n", g_spi_life_test_cycles);
    g_spi_life_test_cycles = 0;

    return ret;
}
/*[HM003-165] Add AT+SPIFLASHLIFETEST command by ganxiao at 20200306 end*/
#endif
